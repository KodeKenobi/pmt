...
import * as routeModule from "@/app/api/analytics/forecast/route";
...
describe("src/app/api/analytics/forecast/route.ts", () => {
  it("GET returns an HTTP response object", async () => {
    const handler = routeModule.GET;
    const request = new NextRequest(
      "http://localhost/api/analytics/forecast?take=10&page=1",
      {
        method: "GET",
