...
import * as routeModule from '@/app/api/auth/logout/route';
...
describe('src/app/api/auth/logout/route.ts', () => {
  it('POST returns an HTTP response object', async () => {
    const handler = routeModule.POST;
    const request = new NextRequest('http://localhost/api/auth/logout?take=10&page=1', {
      method: 'POST',
