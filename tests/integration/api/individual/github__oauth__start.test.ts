...
import * as routeModule from "@/app/api/github/oauth/start/route";
...
describe("src/app/api/github/oauth/start/route.ts", () => {
  it("GET returns an HTTP response object", async () => {
    const handler = routeModule.GET;
    const request = new NextRequest(
      "http://localhost/api/github/oauth/start?take=10&page=1",
      {
        method: "GET",
