...
import * as routeModule from "@/app/api/settings/backups/route";
...
describe("src/app/api/settings/backups/route.ts", () => {
  it("GET returns an HTTP response object", async () => {
    const handler = routeModule.GET;
    const request = new NextRequest(
      "http://localhost/api/settings/backups?take=10&page=1",
      {
        method: "GET",
