...
import * as routeModule from '@/app/api/settings/test-email/route';
...
describe('src/app/api/settings/test-email/route.ts', () => {
  it('POST returns an HTTP response object', async () => {
    const handler = routeModule.POST;
    const request = new NextRequest('http://localhost/api/settings/test-email?take=10&page=1', {
      method: 'POST',
